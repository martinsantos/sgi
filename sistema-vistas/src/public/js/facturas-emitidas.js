...
class FacturasManager {
  ...
  renderTable() {
    ...
    tbody.innerHTML = this.allFacturas.map(factura => `
      <tr>
        ...
        <td>${this.formatDate(factura.fecha_emision)}</td>
        <td>
          <div>${factura.cliente_nombre || 'Sin cliente'}</div>
          <small class="text-muted">${factura.cliente_codigo || ''}</small>
        </td>
        ...
        <td class="text-end">
          <div>${this.formatCurrency(factura.cancelado || 0)}</div>
  ...
  renderPagination(pagination) {
    ...
    // Botón anterior
    if (page > 1) {
      html += `<li class="page-item"><a class="page-link" onclick="window.facturasManager.goToPage(${page - 1})">Anterior</a></li>`;
    } else {
    ...
    for (let i = startPage; i <= endPage; i++) {
      if (i === page) {
        ...
        html += `<li class="page-item"><a class="page-link" onclick="window.facturasManager.goToPage(${i})">${i}</a></li>`;
      }
    }

    // Botón siguiente
    if (page < pages) {
      html += `<li class="page-item"><a class="page-link" onclick="window.facturasManager.goToPage(${page + 1})">Siguiente</a></li>`;
    } else {
